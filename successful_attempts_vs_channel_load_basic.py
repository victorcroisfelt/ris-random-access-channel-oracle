...
access_policy = 'RCURAP'
access_policy = 'RCARAP'
access_policy = 'RGSCAP'
access_policy = 'SMAP'
...
avg_num_successful_attempts = np.zeros((channel_loads.size, num_setups))
avg_num_successful_attempts_unicast = np.zeros((channel_loads.size, num_setups))
avg_num_successful_attempts_multicast = np.zeros((channel_loads.size, num_setups))
...
for cc in trange(channel_loads.size, desc="Channel Load", unit=" chnload"):
    ...
    for ss in trange(num_setups, desc="Setup", unit=" setup"):
        ...
        if num_ues == 0:
            ...
            avg_num_successful_attempts[cc, ss] = np.nan
            avg_num_successful_attempts_unicast[cc, ss] = np.nan
            avg_num_successful_attempts_multicast[cc, ss] = np.nan
        ...
        # Apply access policy
        chosen_access_slots = frame.ac.access_policy(snr, ac_info, num_repetitions=num_repetitions, policy=access_policy)
        ...
        avg_num_successful_attempts[cc, ss] = len(successful_ac_ues) / num_ues
        ...
        # Save results
        avg_num_successful_attempts_unicast[cc, ss] = len(list_successful_ues_unicast) / num_ues
        avg_num_successful_attempts_multicast[cc, ss] = len(list_successful_ues_multicast) / num_ues
...
ax.plot(channel_loads, np.nanmean(avg_num_successful_attempts, axis=-1), linewidth=1.5)
ax.plot(channel_loads, np.nanmean(avg_num_successful_attempts_unicast, axis=-1), linewidth=1.5)
ax.plot(channel_loads, np.nanmean(avg_num_successful_attempts_multicast, axis=-1), linewidth=1.5)
