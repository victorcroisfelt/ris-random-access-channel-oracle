...
for cc in trange(channel_loads.size, desc="Channel Load", unit=" chnload"):
    ...
    for ss in range(num_setups):
        ...
        for ap in range(len(access_policies)):
            ...
            # Get UE mask
            mask_ue = np.arange(0, ac_num_successful_ues)

            # Generate ACK message
            ack_messages = np.sqrt(1/2) * (np.random.randn(ac_num_successful_ues, 1) + 1j * np.random.randn(ac_num_successful_ues, 1))

            # Generate noise
            noise = np.sqrt(1/2) * (np.random.randn(ac_num_successful_ues, 1) + 1j * np.random.randn(ac_num_successful_ues, 1))
            ...
            for aa, ack_method in enumerate(ack_methods):
                ...
                dl_channels = dl_channels[mask_ue, mask_ue]

                # Get number of ACK success
                ack_num_successful_ues = frame.ack.dl_transmission(dl_channels, ack_messages, noise)

                # Store simulation results
                proba_access[aa + 1, ap, cc, ss] = ack_num_successful_ues / num_ues
                #throughput[aa + 1, ap, cc, ss] = frame.compute_throughput(access_policy, ack_num_successful_ues, ack_method=ack_method)
