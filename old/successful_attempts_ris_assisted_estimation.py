...
num_configs_range = np.array([2, 8])
...
# Estimation quality range
estimation_quality_range = np.linspace(0, 1, 11)
...
total_num_successful_attempts = np.empty((estimation_quality_range.size, num_configs_range.size, num_ues_range.size, num_setups))
...
for ss in trange(num_setups, desc="Simulating", unit="setups"):
    ...
    for ii, num_ues in enumerate(num_ues_range):
        ...
        for cc, num_configs in enumerate(num_configs_range):
            ...
            # Go through all estimation qualities
            for ee, estimation_quality in enumerate(estimation_quality_range):
                ...
                configs_strength = np.sqrt((1 - estimation_quality**2)) * channel_gains_dl + estimation_quality * np.sqrt(1 / 2) * (np.random.randn(num_configs, num_ues) + 1j * np.random.randn(num_configs, num_ues))
                ...
                ##################################################
                total_num_successful_attempts[ee, cc, ii, ss] = collision_resolution(ue_choices, buffered_access_attempts, gamma_th)

# Save data
np.savez('data/ris_' + choose_config + '_N' + str(Nx * Nz) + '_estimation.npz',
         estimation_quality_range=estimation_quality_range,
