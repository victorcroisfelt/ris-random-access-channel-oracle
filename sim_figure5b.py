...
accuracy = np.zeros((len(access_policies), range_rec_error.size, range_ac_num_slots.size, num_setups))
...
for asl in trange(range_ac_num_slots.size, desc="access size", unit=" as"):
    ...
    for ap in range(len(access_policies)):
        ...
        true_chosen_ac_slots = frame.ac.access_policy(ac_true_info, access_policy=access_policies[ap], rng=np.random.RandomState(42))
        ...
        for re in range(range_rec_error.size):
            ...
            noisy_chosen_ac_slots = frame.ac.access_policy(ac_info, access_policy=access_policies[ap], rng=np.random.RandomState(42))

            # Go through each UE
            for ue in range(num_setups):
                intersection = (set(true_chosen_ac_slots[ue])).intersection(set(noisy_chosen_ac_slots[ue]))
                accuracy[ap, re, asl, ue] = len(set(intersection)) / len(true_chosen_ac_slots[ue])
...
for ap, access_policy in enumerate(access_policies):
    for re, rec_error in enumerate(range_rec_error):
        ax.plot(range_ac_num_slots, np.mean(accuracy[ap, re, :, :], axis=-1), linestyle=linestyles[ap])

ax.set_xlabel(r'number of access slots, $N_{\mathrm{ac}}$')
