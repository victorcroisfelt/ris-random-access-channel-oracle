...
# Define number of training slots
tr_num_slots = 46
...
# Range of number of repetitions
range_num_repetitions = np.arange(1, 11)
...
# Tuple containing result vector sizes
result_size = (len(access_policies), channel_loads.size, range_num_repetitions.size, num_setups)

# Prepare to store number of successful attempts
avg_num_successful_attempts = np.zeros(result_size)
avg_num_successful_attempts_unicast = np.zeros(result_size)
avg_num_successful_attempts_multicast = np.zeros(result_size)
...
for ap in trange(len(access_policies), desc="Access Policy", unit=" ap"):
    ...
    for cc in trange(channel_loads.size, desc="Channel Load", unit=" chnload"):
        ...
        for ss in range(num_setups):
            ...
            # Go through all number of repetitions
            for rr in range(range_num_repetitions.size):

                # Extract current number of repetitions
                num_repetitions = range_num_repetitions[rr]
                ...
                avg_num_successful_attempts[ap, cc, rr, ss] = len(successful_ac_ues) / num_ues
                ...
                # Save results
                avg_num_successful_attempts_unicast[ap, cc, rr, ss] = len(list_successful_ues_unicast) / num_ues
                avg_num_successful_attempts_multicast[ap, cc, rr, ss] = len(list_successful_ues_multicast) / num_ues

# Squeezing
avg_num_successful_attempts_max = np.nanmax(np.nanmean(avg_num_successful_attempts, axis=-1), axis=2)
avg_num_successful_attempts_unicast_max = np.nanmax(np.nanmean(avg_num_successful_attempts_unicast, axis=-1), axis=2)
avg_num_successful_attempts_multicast_max = np.nanmax(np.nanmean(avg_num_successful_attempts_multicast, axis=-1), axis=2)

print(np.nanargmax(np.nanmean(avg_num_successful_attempts, axis=-1), axis=2) + 1)
print(np.nanargmax(np.nanmean(avg_num_successful_attempts_unicast, axis=-1), axis=2) + 1)
print(np.nanargmax(np.nanmean(avg_num_successful_attempts_multicast, axis=-1), axis=2) + 1)
...
for ap in range(len(access_policies)):
    ...
    axes[0].plot(channel_loads, avg_num_successful_attempts_max[ap], linewidth=1.5, label=access_policies[ap])
    axes[1].plot(channel_loads, avg_num_successful_attempts_unicast_max[ap], linewidth=1.5)
    axes[2].plot(channel_loads, avg_num_successful_attempts_multicast_max[ap], linewidth=1.5)

axes[0].set_xlabel(r'channel load, $\kappa$')
axes[0].set_ylabel(r'successful attempts ratio')
...
axes[1].set_xlabel(r'channel load, $\kappa$')
axes[1].set_ylabel(r'successful attempts ratio')

#ax.set_yscale('log')

axes[1].grid(color='gray', linestyle=':', linewidth=0.5, alpha=0.5)

axes[2].set_xlabel(r'channel load, $\kappa$')
axes[2].set_ylabel(r'successful attempts ratio')
