...
range_rec_error = np.array([np.nan, 10**(-1), 10**(-3)])

# Define the access policies
access_policies = ['RCURAP', 'SMAP']
...
frame.init_access(ac_min_num_slots, num_channel_uses_ul, 0, decoding_snr, 1/2, num_els_hor)
...
for cc in trange(channel_loads.size, desc="Channel Load", unit=" chnload"):
    ...
    for ss in range(num_setups):
        ...
        for re in range(range_rec_error.size):
            ...
            ac_info = ac_true_info.copy()

            if not np.isnan(rec_error):

                # Noisy access info
                ac_info += np.sqrt(rec_error) * rec_noise
            ...
            for ap in range(len(access_policies)):
                ...
                # Apply access policy
                chosen_access_slots = frame.ac.access_policy(ac_info, policy=access_policies[ap])
...
# Save data
np.savez(
    'data/evaluating_dl_training_block_rec_error.npz',
